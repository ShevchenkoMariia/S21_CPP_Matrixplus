...
class S21Matrix {
 ...
 public:
  S21Matrix()
      : rows_(0), cols_(0), matrix_(nullptr) {}  // Дефолтный конструктор.
  S21Matrix(int rows,
            int cols);  // Параметризированный конструктор (строи и столбцы).
  ...
  S21Matrix minorMatrix(S21Matrix* A, int rowDel,
                        int colDel);  // Минор матрицы.
